    ...
    public static class Plane extends Thread {
        ...
        public void run() {
            ...
            while (!isAlreadyTakenOff) {
                if (RUNWAY.trySetTakingOffPlane(this)) {//if t==null,переопределяем//если взлетная полоса свободна, занимаем ее
                    System.out.println(getName() + " взлетает");
                    takingOff();//thread.sleep(100); //взлетает
                    System.out.println(getName() + " уже в небе");
                    isAlreadyTakenOff = true; //переопределяем isAlreadyTakenOff
                    RUNWAY.setTakingOffPlane(null); //обнуляем t в 52 строке
                } else if (!this.equals(RUNWAY.getTakingOffPlane())){//если в t не наш объект вписан //если взлетная полоса занята самолетом
